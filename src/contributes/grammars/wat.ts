  ...
  instrPlainConst(): schema.Rule {
    return {
      ...
      beginCaptures: {
        ...
        capture(seq(opt(Token.sign), words(group(alt(Token.hexfloat, seq("nan:0x", Token.hexfloat)))))),
        capture(seq(opt(Token.sign), Token.float)),
        capture(seq(opt(Token.sign), seq("0x", Token.hexnum))),
        capture(seq(opt(Token.sign), Token.num)),
      ),
      endCaptures: {
        1: { name: "constant.numeric.float.wasm" },
        2: { name: "constant.numeric.float.hexadecimal.wasm" },
        3: { name: "constant.numeric.float.decimal.wasm" },
